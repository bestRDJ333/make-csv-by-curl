...
$tokenObj = $curlClass->getToken(_USERNAME, _PASSWORD);
// getToken
...
while(($line = fgetcsv($f)) !== false ){
  if($num === 0){
    ...
    $email = $line[1];
    // 留言
    $commentsObj = $curlClass->getUserComments( $token, $userId, '2022-01-01', '2022-02-23');
    $data = $commentsObj->data;
    ...
    if($data->num_rows > 0){
      ...
      if($data->num_pages > 1){
        // 1頁以上
        for($i = 2; $i <= $data->num_pages; $i++){
          $commentsObj = $curlClass->getUserComments( $token, $userId, '2022-01-01', '2022-02-23', $i);
          $data = $commentsObj->data;
          $dataArry = array_merge($dataArry, $data->results);
        }
      }
